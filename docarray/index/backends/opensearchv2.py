...
T = TypeVar('T', bound='OpenSearchDocIndex')

OPENSEARCH_PY_VEC_TYPES: List[Any] = [list, tuple, np.ndarray, AbstractTensor]
...
if torch is not None:
    OPENSEARCH_PY_VEC_TYPES.append(torch.Tensor)

if tf is not None:
    from docarray.typing import TensorFlowTensor

    OPENSEARCH_PY_VEC_TYPES.append(tf.Tensor)
    OPENSEARCH_PY_VEC_TYPES.append(TensorFlowTensor)


class OpenSearchDocIndex(BaseDocIndex, Generic[TSchema]):
    _index_vector_params: Optional[Tuple[str]] = ('dimension', 'space_type')
    _index_vector_options: Optional[Tuple[str]] = ('m', 'ef_construction')

    def __init__(self, db_config=None, **kwargs):
        """Initialize OpenSearchDocIndex"""
        super().__init__(db_config=db_config, **kwargs)
        self._db_config = cast(OpenSearchDocIndex.DBConfig, self._db_config)

        self._logger.debug('OpenSearch index is being initialized')

        # OpenSearch client creation
        self._client = OpenSearch(
            hosts=self._db_config.hosts,
            auth=self._db_config.auth,
            **self._db_config.es_config,
        )

        self._logger.debug('OpenSearch client has been created')
        ...
        for col_name, col in self._column_infos.items():
            ...
            if safe_issubclass(col.docarray_type, AnyDocArray):
                continue
            if col.db_type == 'knn_vector' and (
                not col.n_dim and col.config['dimension'] < 0
            ):
                self._logger.info(
        ...
        if self._client.indices.exists(index=self.index_name):
            ...
            self._client_create(
                {'settings': {'index': {'knn': True}}, 'mappings': mappings}
            )
            ...
            self._client_create(
                {
                    'settings': {
                        'index': {'knn': True},
                        **self._db_config.index_settings,
                    },
                    'mappings': mappings,
                }
            )
    ...
    def index_name(self):
        ...
        if default_index_name is None:
            err_msg = (
                'An OpenSearchDocIndex must be typed with a Document type.To do so, use the syntax: '
                'OpenSearchDocIndex[DocumentType] '
            )
    ...
    class QueryBuilder(BaseDocIndex.QueryBuilder):
        ...
        def build(self, *args, **kwargs) -> Any:
            """Build the OpenSearch query object."""
            self._outer_instance._logger.debug('Building the OpenSearch query object')
    ...
    def build_query(self, **kwargs) -> QueryBuilder:
        """
        Build a query for OpenSearchDocIndex.
    ...
    class DBConfig(BaseDocIndex.DBConfig):
        """Dataclass that contains all "static" configurations of OpenSearchDocIndex."""

        hosts: Union[
            List[Union[str, Mapping[str, Union[str, int]]]], str
        ] = 'http://localhost:9200/'
        ...
        def __post_init__(self):
            ...
            self.default_column_config['knn_vector'] = self.knn_vector_config()

        def knn_vector_config(self):
            """Get the dense vector config."""

            config = {
                'dimension': -1,
                'space_type': 'cosinesimil',  # 'l2, 'inner_product', 'cosinesimil'
                'm': 16,
                'ef_construction': 100,
    ...
    class RuntimeConfig(BaseDocIndex.RuntimeConfig):
        """Dataclass that contains all "dynamic" configurations of OpenSearchDocIndex."""
    ...
    def python_type_to_db_type(self, python_type: Type) -> Any:
        ...
        for allowed_type in OPENSEARCH_PY_VEC_TYPES:
            if safe_issubclass(python_type, allowed_type):
                self._logger.info(
                    f'Mapped Python type {python_type} to database type "knn_vector"'
                )
                return 'knn_vector'
        ...
        for type in opensearch_py_types.keys():
            if safe_issubclass(python_type, type):
                self._logger.info(
                    f'Mapped Python type {python_type} to database type "{opensearch_py_types[type]}"'
                )
                return opensearch_py_types[type]
    ...
    def _index(
        ...
        for row in data:
            ...
            for col_name, col in self._column_infos.items():
                if safe_issubclass(col.docarray_type, AnyDocArray):
                    continue
                if col.db_type == 'knn_vector' and np.all(row[col_name] == 0):
    ...
    def execute_query(self, query: Dict[str, Any], *args, **kwargs) -> Any:
        """
        Execute a query on the OpenSearchDocIndex.
    ...
    def _find(
        ...
        body = self._form_search_body(query, search_field, limit)
    ...
    def _create_index_mapping(cls, col: '_ColumnInfo') -> Dict[str, Any]:
        ...
        if col.db_type == 'knn_vector':
            if cls._index_vector_params is not None:
                for k in cls._index_vector_params:
                    index[k] = col.config[k]
            if col.n_dim:
                index['dimension'] = col.n_dim
            if cls._index_vector_options is not None:
                index['method'] = {
                    'parameters': {k: col.config[k] for k in cls._index_vector_options}
                }
                index['method']['name'] = 'hnsw'
        return index
    ...
    def _client_create(self, settings: Dict[str, Any]):
    ...
    def _client_mget(self, ids: Sequence[str]):
        body_ids = [{'_index': self.index_name, '_id': _id} for _id in ids]
        return self._client.mget(index=self.index_name, body={'docs': body_ids})
