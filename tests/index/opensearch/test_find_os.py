...
@pytest.mark.parametrize('similarity', ['cosinesimil', 'l2', 'inner_product'])
def test_find_simple_schema(similarity, tmp_index_name, auth):  # noqa: F811
    class SimpleSchema(BaseDoc):
        tens: NdArray[10] = Field(similarity=similarity)

    index = OpenSearchDocIndex[SimpleSchema](index_name=tmp_index_name, auth=auth)

    index_docs = []
    for _ in range(10):
        vec = np.random.rand(10)
        if similarity == 'inner_product':
...
@pytest.mark.parametrize('similarity', ['cosinesimil', 'l2', 'inner_product'])
def test_find_flat_schema(similarity, tmp_index_name, auth):  # noqa: F811
    class FlatSchema(BaseDoc):
        tens_one: NdArray = Field(dimension=10, similarity=similarity)
        tens_two: NdArray = Field(dimension=50, similarity=similarity)

    index = OpenSearchDocIndex[FlatSchema](index_name=tmp_index_name, auth=auth)
...
@pytest.mark.parametrize('similarity', ['cosinesimil', 'l2', 'inner_product'])
def test_find_nested_schema(similarity, tmp_index_name, auth):  # noqa: F811
    ...
    class DeepNestedDoc(BaseDoc):
        d: NestedDoc
        tens: NdArray = Field(similarity=similarity, dimension=10)

    index = OpenSearchDocIndex[DeepNestedDoc](index_name=tmp_index_name, auth=auth)
...
def test_find_torch():
    ...
    index = OpenSearchDocIndex[TorchDoc]()
...
def test_find_tensorflow():
    ...
    index = OpenSearchDocIndex[TfDoc]()
...
def test_find_batched(tmp_index_name, auth):  # noqa: F811
    index = OpenSearchDocIndex[SimpleDoc](index_name=tmp_index_name, auth=auth)
...
def test_filter():
    ...
    index = OpenSearchDocIndex[MyDoc]()
...
def test_query_builder():
    class MyDoc(BaseDoc):
        tens: NdArray[10] = Field(similarity='l2')
        num: int
        text: str
    ...
    docs, _ = index.execute_query(q)
    assert [doc['id'] for doc in docs] == ['7', '8', '9']
    ...
    assert [doc['id'] for doc in docs] == ['4', '5', '6', '7']
...
def test_contain():
    ...
    index = OpenSearchDocIndex[SimpleSchema]()
