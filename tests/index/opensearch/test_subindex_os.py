...
@pytest.fixture
def index():
    index = OpenSearchDocIndex[MyDoc](index_name='idx')
    my_docs = [
        MyDoc(
            id=f'{i}',
            docs=DocList[SimpleDoc](
...
def test_subindex_init(index):
    assert isinstance(index._subindices['docs'], OpenSearchDocIndex)
    assert isinstance(index._subindices['list_docs'], OpenSearchDocIndex)
    assert isinstance(
        index._subindices['list_docs']._subindices['docs'], OpenSearchDocIndex
    )
...
def test_subindex_contain(index):
    ...
    # Empty index
    empty_index = OpenSearchDocIndex[MyDoc]()
